...
describe("Supabase Utils", () => {
  ...
  it("getSweepById fetches a single sweep joined with logs", async () => {
    const mockData = { id: "123", repo_name: "test-repo", aegisguard-init_logs: [] };
    mockSingle.mockResolvedValue({ data: mockData, error: null });

    const { data } = await getSweepById("123");

    expect(mockFrom).toHaveBeenCalledWith("aegisguard-init_sweeps");
    expect(mockSelect).toHaveBeenCalledWith("*, aegisguard-init_logs ( * )");
  ...
  it("insertLog inserts a log entry", async () => {
    ...
    expect(mockFrom).toHaveBeenCalledWith("aegisguard-init_logs");
