...
export async function getSweepById(id: string) {
  const { data, error } = await createServerClient()
    .from("aegisguard-init_sweeps")
    .select("*, aegisguard-init_logs ( * )")
    .eq("id", id)
    .single();
  return { data: data as (SweepRow & { aegisguard-init_logs: LogRow[] }) | null, error };
}
...
export async function insertLog(row: Pick<LogInsert, "sweep_id" | "level" | "message">) {
  return createServerClient().from("aegisguard-init_logs").insert(row);
}
